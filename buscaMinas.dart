...
void placeMines(List<List<Cell>> board) {
  ...
  while (placedMines < minesCount) {
    ...
    int row = quadrant == 0 ? random.nextInt(rows ~/ 2) : rows ~/ 2 + random.nextInt(rows ~/ 2);
...
bool revealCell(List<List<Cell>> board, int row, int col) {
  ...
  if (row < 0 || row >= rows || col < 0 || col >= cols || board[row][col].isRevealed || board[row][col].hasFlag) {
    return false;
  }
  ...
  if (board[row][col].adjacentMines == 0) {
    for (int dr = -1; dr <= 1; dr++) {
      for (int dc = -1; dc <= 1; dc++) {
        if (dr != 0 || dc != 0) { // Evitar la casilla actual
